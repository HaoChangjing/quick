 ...
 */
@Slf4j
public class FastJsonRedisRepository extends RedisTemplate<String, String> {
	/**
	 * 默认编码
	 */
	...
	public FastJsonRedisRepository() {
		this.setKeySerializer(STRING_SERIALIZER);
		this.setValueSerializer(OBJECT_SERIALIZER);
	}
	...
	public void setExpire(final String[] keys, final Object[] values, final long time) {
		...
		for (int i = 0; i < keys.length; i++) {
			byte[] bKeys = STRING_SERIALIZER.serialize(keys[i]);
			byte[] bValues = OBJECT_SERIALIZER.serialize(values[i]);
			redisConnection.setEx(bKeys, time, bValues);
		}
		/*this.execute((RedisCallback<Long>) connection -> {
			RedisSerializer<String> serializer = getRedisSerializer();
			for (int i = 0; i < keys.length; i++) {
	...
	public Set<String> keys(final String keyPatten) {
		return this.execute((RedisCallback<Set<String>>) connection -> this.keys(keyPatten + "*"));
	}
...
*/
	/**
	 * @Description 设置某个键过期时间
	 * @MethodName expire
	 **/
	public Boolean expire(String key, long timeOut, TimeUnit timeUnit){
		return this.expire(key,timeOut,timeUnit);
	}
}
